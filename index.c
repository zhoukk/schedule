...
int index_regist(struct index *idx, void *ud) {
	int i;
	if (!ud) return 0;
...
void *index_grab(struct index *idx, int id) {
	struct slot *slot;
	void *ud;
	if (id == 0) return 0;
...
void *index_release(struct index *idx, int id) {
	struct slot *slot;
	void *ud = 0;
	if (id == 0) return 0;
	rwlock_rlock(&idx->lock);
...
int index_list(struct index *idx, int n, int *list) {
	int i, cnt = 0;
	rwlock_rlock(&idx->lock);
